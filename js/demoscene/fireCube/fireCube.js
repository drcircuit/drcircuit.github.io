let parent = document.getElementById("3dcube");
let scr = dcl.setupScreen(320,320, false, 1, parent);
...
function draw() {
    ...
    cube.vertices.forEach(function (v, i) {
        cube.transformedVertices[i] = v.rotateX(angle).rotateY(angle).project(scr.width, scr.height, scr.height /.8, 8.5);
    });
    ...
    for(let y = 0; y<scr.height;y+=f){
        for(let x = 0;x<scr.width;x+=f){
            ...
            if(pr > 64 && pa >0){
                ...
                flame[fy][fx] =c;
                id.data[idx] = palette[180].r;
                id.data[idx+1] = palette[180].g;
                id.data[idx+2] = palette[180].b;
                id.data[idx+3] = 255;
            }
        }
    }
